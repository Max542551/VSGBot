...
from database.get_to_db import get_delivery_by_id, get_order_by_id, get_taxi
from keyboards.inline.delivery_inline.confirm_button_delivery import delivery_send_confirmation_buttons
...
async def process_accept_price_callback(callback_query: types.CallbackQuery):
    _, order_id, taxi_id, price, delivery = callback_query.data.split("_", 4)
    ...
    if delivery == "True":
        ...
        await bot.send_message(
            chat_id=taxi_id,
            text=f"🥳 Пользователь принял ваше предложение по цене {price} руб!", reply_markup= delivery_send_confirmation_buttons(taxi_id, order_id, price))
