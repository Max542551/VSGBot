...
async def order_request(call: types.CallbackQuery, state: FSMContext):
    ...
    await call.message.answer(
        f"<b>ВАШ ЗАКАЗ ПОЛУЧАТ {count_deliveries} ВОДИТЕЛЕЙ</b>\n\n⚠️ Вы создаете заказ на доставку. Средняя стоимость доставки +100 (200) руб. к стоимости такси, цена может измениться индивидуально по предварительной договорённости сторон. После принятия вашего заказа, обязательно свяжитесь с водителем по телефону и убедитесь в его готовности выполнения заказа. Администрация бота не несёт ответственности за выполнение заказов. Ответственными за выполнение заказа являетесь только Вы как заказчик и водитель.\n\n" 
...
async def handle_comment(message: types.Message, state: FSMContext):
    ...
    if order: 
        message_text = f"⚠️Заказ создан. Вам будут поступать предложения от водителей с ценой за услугу доставки (без учёта стоимости покупки, если её оплатил водитель (оплачивается по чеку)) После принятия вашего заказа, обязательно свяжитесь с водителем по телефону и убедитесь в его готовности выполнения заказа.\n\n" \
