...
from database.get_to_db import get_delivery_by_id, get_taxi
...
from states.delivery_states import DeliveryProposePriceState
from states.order_states import ProposePriceState


@dp.callback_query_handler(lambda c: c.data and c.data.startswith('delivery_order_propose_price'))
async def process_propose_price_callback(callback_query: types.CallbackQuery):
    ...
    await state.set_state(DeliveryProposePriceState.price)
...
@dp.message_handler(state=DeliveryProposePriceState.price, content_types=types.ContentType.TEXT)
async def process_propose_price(message: types.Message, state: FSMContext):
    ...
    try:
        ...
        message = await bot.send_message(order.user_id,
                                        ...
                                        reply_markup=accept_or_decline_price_keyboard(order, taxi_id, price, "True"),
                                        parse_mode='html')
        # Save sent message
        save_sent_delivery_messages(order.id, [(order.user_id, message.message_id)])
