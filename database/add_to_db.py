...
from database.database import SentDeliveryItem, SentDeliveryMessage, User, Taxi, Order, SentMessage, SentItem, Delivery
...
def create_delivery_in_db(order_data):

    Delivery.create(
        user_id=order_data["user_id"],
        first_address=order_data["first_address"],
        second_address=order_data["second_address"],
        package_payment=order_data["package_payment"],
        package_content=order_data["package_content"],
        package_price=order_data["package_price"] if order_data.get("package_price") else None,
        payment_method=order_data["payment_method"],
        comment=order_data["comment"],
        cost=order_data["cost"],
    )
...
def save_sent_delivery_messages(order_id, sent_messages):
    order = Delivery.get(Delivery.id == order_id)
    for user_id, message_id in sent_messages:
        SentDeliveryMessage.create(order=order, user_id=user_id, message_id=message_id)
...
async def delivery_create_sent_item(order):
    sent_item = SentDeliveryItem.get_or_none(order=order)
    if sent_item is None:
        sent_item = SentDeliveryItem.create(order=order)
    return sent_item
